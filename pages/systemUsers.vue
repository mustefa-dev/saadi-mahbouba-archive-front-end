...
const { data: adminsData, refresh: fetchAdmins, pending: isLoading } = await useFetch<UsersResponse>(
  apiPaths.users,
  {
    key: 'admins-systemusers',
    lazy: true,
    query: computed(() => ({
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <BaseParagraph size="sm" class="text-muted-400">
          إدارة حسابات المشرفين والإداريين
        </BaseParagraph>
      </div>
    </div>
            ...
            <tr v-for="admin in admins" :key="admin.id">
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center gap-3">
                  <div class="flex items-center justify-center size-10 rounded-full bg-primary-500/10 text-primary-500">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center gap-2">
                  <EditUser :user="admin" @edited="fetchAdmins" />
                  <DeleteUser
                    :user-id="admin.id"
