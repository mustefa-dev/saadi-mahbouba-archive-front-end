...
import { managersTableHeaders } from '..';
...
const loadData = async ()=>{
  ...
  const res = await axiosIns.get(`dashboard/manager-ticketsnumber?pageSize=5&pageNumber=${pageNumber.value}`);
...
<template>
  <div class="flex flex-col gap-2">
    <span class="text-md font-bold">
      احصائيات شكاوي الموظفين
    </span>
    <div class="flex-grow h-full">
      <AppTable :items="users" :headers="managersTableHeaders">
        <BasePagination
          :item-per-page="5"
