...
const deleteReport = async () => {
  ...
  try {
    await $fetch(apiPaths.reportById(props.reportId), {
      method: 'DELETE'
    });

    helpers.setSuccessMessage('ar', 'Report deleted successfully', 'تم حذف التقرير بنجاح');
    isOpen.value = false;
    emit('deleted');
  } catch (error: any) {
    helpers.setErrorMessage(error, 'ar', 'Failed to delete report', 'فشل حذف التقرير');
  } finally {
